...
const Form = () => {
	...
	//Once clicked, setTicker(ticker). On Change, setTicker['']

	function autofill() {
		//1. Data
		useEffect(() => {
			const fetchAlphaData = async () => {
				const url = `https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY_ADJUSTED&symbol=TSLA&apikey=S5EXI4C9NV70PN3A`;
				try {
		...
		// 2. Find most recent price using most recent date.
		const data = individualStock['Weekly Adusted Time Series'];
		// console.log(data);

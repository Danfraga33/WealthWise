...
const Form = () => {
	...
	useEffect(() => {
		const fetchAlphaData = async () => {
			const url = `https://api.polygon.io/v2/aggs/ticker/ACN/range/1/day/2023-09-12/2023-09-13?adjusted=false&sort=asc&limit=120&apiKey=WsJDp42rPMioim4r9xtNIK3GLoRpi1z5`;
	...
	// const { results } = individualStock;
	const currentPriceArr = individualStock;
	// const currentPrice = currentPriceArr.map((date) => {
	// 	console.log(date);
	// });
	const currentPrice = currentPriceArr.results.map((date) => {
		return date.c;
	});
	console.log(currentPrice.results);

	// const weeklyAdjustedTimeSeries = individualStock
	// 	? individualStock['Weekly Adjusted Time Series']
	// 	: console.log('It HAPPENED');

	// console.log(data);
	// console.log(weeklyAdjustedTimeSeries);

	// let mostRecentDate = null;
	// let mostRecentData = null;

	// for (const date in weeklyAdjustedTimeSeries) {
	// 	if (!mostRecentDate || date > mostRecentDate) {
	// 		mostRecentDate = date;
	// 		mostRecentData = weeklyAdjustedTimeSeries[date];
	// 	}
	// }

	// const mostRecentPrice = Number(mostRecentData['4. close']);
	// console.log(typeof mostRecentPrice);
	// console.log(mostRecentPrice);
	// const mostRecentPrice = mostRecentData?.mostRecentData['4. close'];

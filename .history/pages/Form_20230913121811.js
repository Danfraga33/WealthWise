...
const Form = () => {
	...
	useEffect(() => {
		const fetchAlphaData = async () => {
			const url = `https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/day/2023-09-12/2023-09-13?adjusted=false&sort=asc&limit=120&apiKey=WsJDp42rPMioim4r9xtNIK3GLoRpi1z5`;
	...
	const currentPrice = individualStock;
	// const currentPrice = currentPriceArr.map((date) => {
	// 	console.log(date);
	// });
	currentPrice.results.map((date) => {});

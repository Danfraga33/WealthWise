...
const settings = () => {
	...
	function updateStates() {
		...
		console.log(typeof newGrowthRate, newGrowthRate);
		...
		function calculateCompoundInterest(
			...
			const weeklyInterestRate =
				(1 + annualInterestRate / (compoundingFrequency * 100)) **
		...
		const init123 = 1000000; // Initial investment
		console.log(typeof init123);
		const finalAmount = 5000000; // Desired final value
		const annualInterestRate = 10; // Annual interest rate (5%)
		const compoundingFrequency = 12; // Weekly compounding

		const valuesArray = calculateCompoundInterest(
			init123,
			finalAmount,
			annualInterestRate,
			compoundingFrequency

...
import fetchAlphaData from './StockAPI';
...
const Form = () => {
	...
	useEffect(() => {
		const fetchAlphaData = async (ticker) => {
			const url = `https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY_ADJUSTED&symbol=${ticker}&apikey=S5EXI4C9NV70PN3A`;
			try {
	...
	// console.log(individualStock['Weekly Adjusted Time Series']);
	const data = individualStock;
	// console.log(data);
	...
	console.log('Data for most recent date:', mostRecentData['4. close']);
										...
										{suggestions.map((suggestion) => (
											<li
												...
												onClick={() => {
													...
													fetchAlphaData(suggestion.ticker);
													setSuggestions([]);
													// console.log(fetchAlphaData);
												}}
											>

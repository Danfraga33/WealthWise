...
const settings = () => {
	...
	function calculateCompoundInterest(initialAmount, finalAmount, annualInterestRate, compoundingFrequency) {
		const weeksInYear = 52;
		const weeklyInterestRate = (1 + annualInterestRate / (compoundingFrequency * 100)) ** (1 / compoundingFrequency) - 1;
		
		let currentAmount = initialAmount;
		const valueArray = [initialAmount];
  
		while (currentAmount < finalAmount) {
			 currentAmount *= (1 + weeklyInterestRate);
			 valueArray.push(currentAmount);
		}
  
		return valueArray;
  }
  
  const initialAmount = 1000000; // Initial investment
  const finalAmount = 5000000;   // Desired final value
  const annualInterestRate = 5;  // Annual interest rate (5%)
  const compoundingFrequency = 52; // Weekly compounding
  
  const valuesArray = calculateCompoundInterest(initialAmount, finalAmount, annualInterestRate, compoundingFrequency);
  
  console.log(valuesArray);
  This code defines a calculateCompoundInterest function that takes the initial amount, desired final amount, annual interest rate, and compounding frequency as inputs. It then calculates the values in an array with weekly compounding until the final amount is reached.
  
  Make sure to run this code in a JavaScript environment, such as a browser console or Node.js, to see the array of values as the investment grows weekly until it reaches $5,000,000.
	...
	const growth = Number(annualGrowthRate);

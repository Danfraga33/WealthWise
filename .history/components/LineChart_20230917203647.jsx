		...
		try {
			...
			if (response.ok) {
				...
				function extractFromPointToEnd(obj, startingKey) {
					...
					return subset;
					return { [ticker]: weeklyClosePrices };
				}

				/////////////////////THIS IS WHAT YOURE LOOKING FOR - NEED WEEKLYCLOSE FOR NEXT STOCK
				console.log(timeSeriesData);
				const random3 = timeSeriesData.map((stock) => {
			...
			try {
				const fetchResults = await Promise.all(
					portfolioData.map(async (stock) => {
						...
						const result = await getTimeSeriesData(ticker);
						return result;
					})
				);
				// console.log('fetchResults:', fetchResults);
				const combinedData = fetchResults.reduce((acc, stockData) => {
					return [...acc, ...stockData];
				}, []);

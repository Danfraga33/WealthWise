		...
		try {
			...
			if (response.ok) {
				...
				function calculateCumulativePricesFromStartDate(data, startDate) {
					...
					let isCumulationStarted = false;

					for (const date in data) {
						if (date === startDate) {
							isCumulationStarted = true;
						}

						if (isCumulationStarted) {
							cumulativePrices[date] = Object.keys(data).reduce((sum, key) => {
								if (key <= date) {

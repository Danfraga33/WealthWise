	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const today = new ; // Assuming today is the current date
				...
				portfolioData.forEach((stock) => {
					const stockDate = new Date(stock.dateOfPurchase);
					const index = entries.findIndex(
						(entry) => entry.date === stockDate.toISOString().split('T')[0]
					);
					if (index !== -1) {
						entries[index].value = stock.marketValue; // Update the value with stock data
					}
				});

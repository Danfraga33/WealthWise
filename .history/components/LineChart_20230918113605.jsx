	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const chartData = Object.values(obj3);
				// console.log(values);
				...
				function calculateSMA(values, period) {
					if (
						...
						throw new Error('Invalid input data or period.');
					}
					...
					for (let i = 0; i < values.length; i++) {
						if (i < period - 1) {
							...
							if (i >= period) {
								sum -= values[i - period]; // Remove the oldest value from the sum
							}
				...
				const movingAverageData = calculateSMA(values, movingAveragePeriod);

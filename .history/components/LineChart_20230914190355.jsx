...
const LineChart = ({ portfolioData }) => {
	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const keyValueArray = Object.entries(weeklyClosePrices);
				...
				// const reversed = weeklyClosePrices.reverse();
				setTest(reversedObject);
	...
	const extractedData = extractFromPointToEnd(test, startingPoint);
	// console.log(userMarketValues);

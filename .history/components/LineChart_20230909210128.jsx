...
const LineChart = (props) => {
	...
	const targetAge = 35;
	const currentAge = 26;
	const periods = targetAge - currentAge;

	function data() {
		return Array.from({ length: periods }, (_, i) => {
			return Math.round(initialAmount * Math.pow(1 + growthRate, i));.Math.round(
			(targetAmount - initialAmount) / (targetAge - currentAge) / periods
		);
	}
	console.log(data());

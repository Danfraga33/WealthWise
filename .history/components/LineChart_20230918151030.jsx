	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const updatedWeeklyPricedDatesWithClose = portfolioData.map((stock) => {
					return weeklyPricedDatesWithClose.map((dateObj) => {
						return {
							id: stock.id, // Add the 'id' property
							ticker: stock.ticker, // Add the 'ticker' property
							position: stock.positionSize, // Add the 'position' property
							...dateObj, // Include the existing properties from dateObj
						};
					});
				});
				...
				// Filter out duplicates and convert the Set back to an array
				const uniqueArray = [...updatedWeeklyPricedDatesWithClose].filter(
					(obj) => {
						// Convert the object to a JSON string and use it as the key in the Set
						const key = JSON.stringify(obj);

						// Check if this key (object) is already in the Set
						if (!uniqueObjects.has(key)) {
							uniqueObjects.add(key); // Add the key to the Set to mark it as seen
							return true; // Include the object in the filtered result
						}

						return false; // Exclude the object as it's a duplicate
					}
				);
				...
				// function multiplyPricesByPosition(data, position) {
				...
				// 			for (const date in dateObj) {
				// 				// Multiply the price by the position
				// 				const price = parseFloat(dateObj[date]);
				// 				multipliedData[date] = (price * position).toFixed(2);
				// 			}
				...
				// // Example usage:
				// const multipliedData = multiplyPricesByPosition(
				// 	updatedWeeklyPricedDatesWithClose,
				// 	122
				// ); // Replace 122 with the desired position size

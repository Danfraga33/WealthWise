	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const userMarketValues1 = weeklyPricedDatesWithClose.map((datesObj) => {
					const date = Object.keys(weeklyPricedDatesWithClose)[0];
					const closePrice = parseFloat(datesObj[date]['4. close']);
					return tickerPortfolio === tickerTimeSeries ? closePrice : 0;
				});

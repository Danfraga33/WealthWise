		...
		try {
			...
			if (response.ok) {
				...
				function calculateCumulativePrices(stockData, startDate) {
					...
					// Initialize the cumulative price to zero
					...
					Object.keys(stockData[0]).forEach((date) => {
						...
						if (date >= startDate) {
							...
							stockData.forEach((stock) => {
								...
								if (stock[date]) {
									// Add the close price to the cumulative price
									cumulativePrice += parseFloat(stock[date]['4. close']);
								}
							});
						}

						// Store the cumulative price for the current date
						cumulativePrices[date] = cumulativePrice;
					});

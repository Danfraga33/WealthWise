		...
		try {
			...
			if (response.ok) {
				...
				function multiplyPricesByPosition(data, position) {
					// Check if data is an array of objects and position is a number
					if (
						Array.isArray(data) &&
						data.length > 0 &&
						...
						data.forEach((dateObj) => {
							...
							for (const date in dateObj) {
								...
								resultData[date] = (price * position).toFixed(4);
							}

							// Check if the resultData object is not a duplicate
							const isDuplicate = multipliedData.some(
								(obj) => JSON.stringify(obj) === JSON.stringify(resultData)
							);

							if (!isDuplicate) {
								multipliedData.push(resultData);
							}
						});

	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const positionSizeMap = {};

				portfolioData.forEach((stock) => {
					const ticker = stock.ticker;
					const position = stock.positionSize;

					if (!positionSizeMap[position]) {
						positionSizeMap[position] = {
							position: position,
							dates: {},
						};
					}

					weeklyPricedDatesWithClose.forEach((dateObj) => {
						// Add date and price information for each position size
						const date = Object.keys(dateObj)[0];
						const price = dateObj[date];

						if (!positionSizeMap[position].dates[date]) {
							positionSizeMap[position].dates[date] = price;
						} else {
							// If the date already exists, add the price to the existing value
							positionSizeMap[position].dates[date] = (
								parseFloat(positionSizeMap[position].dates[date]) +
								parseFloat(price)
							).toFixed(4);
						}
					});
				...
				// Convert the position size map into an array of objects
				const updatedWeeklyPricedDatesWithClose =
					Object.values(positionSizeMap);

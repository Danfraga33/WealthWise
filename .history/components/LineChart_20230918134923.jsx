	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const distinctPositionSizes = [
					...new Set(portfolioData.map((stock) => stock.positionSize)),
				];

				const positionSizeArrays = distinctPositionSizes.map((positionSize) => {
					const combinedData = {};

					weeklyPricedDatesWithClose.forEach((dateObj) => {
						const date = Object.keys(dateObj)[0];
						combinedData[date] = dateObj[date];
					});
				...
				console.log(positionSizeArrays);

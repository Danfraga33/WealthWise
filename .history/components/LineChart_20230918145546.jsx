	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				// Function to check for duplicate objects based on all properties
				function isDuplicate(obj, index, self) {
					return (
						index ===
						self.findIndex((item) => {
				...
				const uniqueUpdatedWeeklyPricedDatesWithClose =
					updatedWeeklyPricedDatesWithClose.filter(isDuplicate);

				console.log(uniqueUpdatedWeeklyPricedDatesWithClose);

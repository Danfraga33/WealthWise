	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const weeklyPricedDatesWithClose = allFridayDates.map((dateObj) => {
					const dateObj = extractFromPointToEnd(weeklyClosePrices, date);
					return Object.fromEntries(
						Object.entries(dateObj).map(([date, data]) => [
							date,
							data['4. close'],
						])
					);

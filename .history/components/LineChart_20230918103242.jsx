	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const userMarketValues1 = Object.keys(weeklyPricedDatesWithClose).map(
					(date) => {
						if (tickerPortfolio === tickerTimeSeries) {
							const values = weeklyPricedDatesWithClose.map(stock => {
								
							}
								
								Object.values(weeklyPricedDatesWithClose);
							console.log(values);

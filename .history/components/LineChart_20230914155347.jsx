...
const LineChart = ({ portfolioData }) => {
	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const reversedValues = userMarketValues.reverse();
				console.log(reversed);
				setUserMarketValues(reversedValues);
	...
	for (let i = 0; i < xAxisLabels.length; i++) {
		marketValueObject[xAxisLabels[i]] = reversedValues[i];
	}
	...
	const marketValue = [];
	const values = Object.values(reversedValues);

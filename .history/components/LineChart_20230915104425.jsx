...
const LineChart = ({ portfolioData }) => {
	...
	const netWorthPriceComp = Object.keys(random).map((date) => {
		const closePrice = parseFloat(random[date]['4. close']);

		return closePrice;
	});
	// console.log(netWorthPriceComp);
	...
	const marketValue = [];
	const netWorth = netWorthPriceComp.map((price) => {
		const currentNetWorth = price;
	...
	useEffect(() => {
		if (startIndex !== -1) {
			// Use slice to create a new array starting from startIndex to the end
			const newRangeOfPrices = netWorthPriceComp.slice(startIndex);
		} else {
			// console.log('Earliest date not found in the array.');
		}
	}, []);

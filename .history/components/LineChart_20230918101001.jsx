	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				function calculateCumulativePrices(stockData, startDate) {
					// Create an object to store cumulative prices for each date
					const cumulativePrices = {};

					// Initialize the cumulative price for each stock
					const cumulativePricesByStock = new Array(stockData.length).fill(0);

					// Iterate over the dates
					Object.keys(stockData[0]).forEach((date) => {
						// Check if the date is after or equal to the start date
						if (date >= startDate) {
							// Reset the cumulative prices for each stock
							cumulativePricesByStock.fill(0);
						}

						// Calculate cumulative prices for each stock for the current date
						stockData.forEach((stock, index) => {
							if (stock[date]) {
								const closePrice = parseFloat(stock[date]['4. close']);
								if (!isNaN(closePrice)) {
									cumulativePricesByStock[index] += closePrice;
								}
							}
						});

						// Store the cumulative prices for each stock in the result object
						cumulativePrices[date] = [...cumulativePricesByStock];
					});

					return cumulativePrices;
				}

				const cumulativePrices = calculateCumulativePrices(
				...
				const userMarketValues1 = Object.keys(random).map((date) => {
					if (tickerPortfolio === tickerTimeSeries) {
						...
						const networth = closePrice1;

						return networth;
					}
					return 0;
				});
				/////////////I HAVE AN ARRAY OF TWO ELEMENTS, EACH ELEMENT HAS PRICES ACCORDING TO DATES.  USERMARKETVALUES1 RETURNS PRICE FROM 2013-01-25. THE 2ND ELEMENT IN WEEKLYPRICEDDATESWITHCLSOE ARRAY, HAS PRICE STARTING FROM 2021-01-29. WHAT I WANT IS THAT EVERY PRICE FOR EACH DAY BEFORE 2021-01-29 UNTIL 2013-01-25, TO BE ZERO. AND WHEN 2021-01-29 HITS, PRICE DATA BEGINS TO SHOW. Write me a fuction where price = value of object for the stock with data starting from 2013-01-25. Then when 2021-01-29 is reached, price from stock2 comes in so that price variable = the price of the stock1 + stock2.

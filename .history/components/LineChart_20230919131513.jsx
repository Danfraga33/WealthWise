...
const LineChart = ({ portfolioData, contextCtx }) => {
	...
	const { initialAmount, targetAmount, annualGrowthRate } = contextCtx;

	// console.log(initialAmount, targetAmount, annualGrowthRate);
		...
		try {
			...
			if (response.ok) {
				...
				function calculateCompoundInterest(
					...
					const weeklyInterestRate = annualInterestRate / 52; // Assuming 52 weeks in a year
					const growthData = [];
					let currentAmount = initialAmount;

					for (let week = 1; week <= timeFrameInWeeks; week++) {
						currentAmount += currentAmount * (weeklyInterestRate / 100); // Calculate growth
						growthData.push({
							week: week,
							amount: currentAmount.toFixed(2), // Round to 2 decimal places
						});

						if (currentAmount >= targetAmount) {
							break; // Target amount reached before the time frame
						}
					}
				...
				// Example usage:
				const initialAmount = 1000; // Initial amount in dollars
				const targetAmount = 1500; // Target amount in dollars
				const annualInterestRate = 5; // Annual interest rate (5%)
				const timeFrameInWeeks = 52; // Expected time frame in weeks

				const growthData = calculateCompoundInterest(
					initialAmount,
					targetAmount,
					annualInterestRate,
					timeFrameInWeeks

	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const latestDate = new Date(); // Assuming you have your latestDate object

				const year = latestDate.getFullYear();
				const month = String(latestDate.getMonth() + 1).padStart(2, '0'); // Month is 0-indexed, so we add 1
				const day = String(latestDate.getDate()).padStart(2, '0');

				const todaysDate = `${year}-${month}-${day}`; // You need to implement a function to find the latest date
				console.log(todaysDate);
				function createArrayOfDates(startDate, todaysDate) {
					const dateArray = [];
					let currentDate = new Date(startDate);

					// Convert the end date to a date object
					const finalDate = new Date(todaysDate);

	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const dateOfPurchaseNewStock = portfolioData[1].dateOfPurchase;
				console.log(startingPoint);
				console.log(portfolioData[1].dateOfPurchase);
				if (portfolioData[0].ticker === tickerTimeSeries) {
					console.log(
						extractFromPointToEnd(
							weeklyClosePrices,
							portfolioData[1].dateOfPurchase
						)
					);
				}

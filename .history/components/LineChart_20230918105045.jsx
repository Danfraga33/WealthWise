	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const array1 = weeklyPricedDatesWithClose[0];
				const array2 = weeklyPricedDatesWithClose[1];
				console.log(weeklyPricedDatesWithClose[0]);
				...
				const tester = Object.keys(weeklyPricedDatesWithClose).map((date) => {
					if (tickerPortfolio === tickerTimeSeries) {
						const values = Object.values(weeklyPricedDatesWithClose);

						const closePrices = Object.values(
							parseFloat(weeklyPricedDatesWithClose[date])
						);
						// console.log(closePrices);
						const networth = closePrices;
				...
				const userMarketValues1 = Object.keys(random).map((date) => {
					if (tickerPortfolio === tickerTimeSeries) {
						const values = Object.values(random);
						// console.log(values);

...
const LineChart = ({ portfolioData }) => {
	// console.log(portfolioData[0].dateOfPurchase);
	const ticker = portfolioData[0].ticker;
	const position = portfolioData[0].position;
	const dateOfPurchase = portfolioData[0].dateOfPurchase;

	// Time Series Data
	async function getTimeSeriesData(ticker) {
		const url = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${ticker}&outputsize=full&apikey=demo`;
		...
		try {
			...
			if (response.ok) {
				...
				const dailyClosePrice = fetchedData['Time Series (Daily)'];
				...
				const userMarketValues = Object.keys(dailyClosePrices).map((date) => {
					...
					return closePrice * position; // Multiply by user's position})

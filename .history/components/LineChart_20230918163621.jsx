	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				onst multipliedData = updatedWeeklyPricedDatesWithClose.map((stock) => {
					const multipliedPrices = {};
					
					// Iterate through the properties of the object (which are dates)
					for (const date in stock) {
					  if (date === 'id' || date === 'ticker' || date === 'position') {
						 // Skip id, ticker, and position properties
						 continue;
					  }
				 
					  // Multiply the price by the position size
					  const price = parseFloat(stock[date]);
					  const positionSize = parseFloat(stock.position);
					  multipliedPrices[date] = (price * positionSize).toFixed(4);
					}
				 
					// Return a new object with id, ticker, position, and multiplied prices
					return {
					  id: stock.id,
					  ticker: stock.ticker,
				...
				// // Example usage:
				const multipliedData = updatedWeeklyPricedDatesWithClose.map(
					(stock) => {
						const positions = stock.positionSize;
						// console.log(positions);
						return multiplyPricesByPosition(stock, positions);
					}
				);

...
const LineChart = ({ portfolioData }) => {
	...
	const earliestDate = portfolioData.map((data) => {
		return data;
	});

	if (earliestDate.length > 0) {
		const firstDate = earliestDate[0];
		console.log(firstDate.dateOfPurchase); // Outputs the first date without it being in an array
	} else {

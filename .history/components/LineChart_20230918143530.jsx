			...
			if (response.ok) {
				...
				portfolioData.forEach((stock) => {
					...
					weeklyPricedDatesWithClose.forEach((dateObj) => {
						...
						if (!positionSizeMap[position][date]) {
							positionSizeMap[position][date] = price;
						} else {
							// If the date already exists, add the price to the existing value
							positionSizeMap[position][date] = (
								parseFloat(positionSizeMap[position][date]) + parseFloat(price)
							).toFixed(4);
						}
					});
				});

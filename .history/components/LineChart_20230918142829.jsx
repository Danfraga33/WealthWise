	...
	async function getTimeSeriesData(ticker) {
		...
		try {
			...
			if (response.ok) {
				...
				const updatedWeeklyPricedDatesWithClose = portfolioData.flatMap(
					(stock) => {
						const ticker = stock.ticker;
						const position = stock.positionSize;
						return weeklyPricedDatesWithClose.map((dateObj) => {
				...
				function multiplyPricesByPosition(data, portfolioData) {
					// Create an object to store aggregated data
					const aggregatedData = {};
				 
					// Check if data is an array of objects and portfolioData is an array
					if (
					  Array.isArray(data) &&
					  data.length > 0 &&
					  ...
					  // Iterate through the portfolio data
					  portfolioData.forEach((stock) => {
						 ...
						 data.forEach((dateObj) => {
							for (const date in dateObj) {
							  // Check if the date is in the aggregated data
							  if (!aggregatedData[date]) {
								 aggregatedData[date] = { position: 0 };
							  }
				 
							  // Check if the ticker is in the aggregated data
							  if (!aggregatedData[date][ticker]) {
								 aggregatedData[date][ticker] = 0;
							  }
				 
							  // Multiply the price by the position and add it to the aggregated data
							  const price = parseFloat(dateObj[date]);
							  aggregatedData[date][ticker] += price * position;
				 
							  // Update the position for the date
							  aggregatedData[date].position = position;
							}
						 });
					  });
					  ...
					  for (const date in aggregatedData) {
						 resultArray.push({ date, ...aggregatedData[date] });
					  }
				 ...
				 // Example usage:
				 const multipliedData = multiplyPricesByPosition(updatedWeeklyPricedDatesWithClose, portfolioData); // Use your portfolioData array here
				 console.log(multipliedData);
				 This function now aggregates the data for the same dates based on your portfolio data, and it will result in an array with two objects for each date, one for each ticker, along with the position size. Replace portfolioData in the example usage with your actual portfolio data array.
